...
assertSame('^1.0', $composer['require']['evolution-cms/evo-ui'] ?? null, 'composer evo-ui dependency uses release constraint');
...
assertContains("'/assets/ssettings.css' => public_path('assets/modules/ssettings/ssettings.css')", $serviceProvider, 'manager stylesheet is publishable as a module asset');
...
assertContains("Route::get('assets/{file}', [sSettingsController::class, 'asset'])", $routes, 'manager stylesheet route supports symlinked package development');
...
assertContains('function asset(string $file): Response', $controller, 'controller serves the module-owned stylesheet');
assertContains("'Content-Type' => 'text/css; charset=UTF-8'", $controller, 'stylesheet route returns CSS content type');
...
$settingsStyles = (string) file_get_contents(dirname(__DIR__) . '/assets/ssettings.css');
assertContains('data-ssettings-compact-values', $settingsPanel, 'compact values root');
...
assertContains('ssettings-compact-row__usage', $settingsPanel, 'values usage hint');
assertContains('ssettings-compact-row__description', $settingsPanel, 'values field description');
assertContains('white-space: nowrap', $settingsStyles, 'values usage hints stay one-line');
assertContains('minmax(260px, 430px)', $settingsStyles, 'values usage column is wide enough for long config keys');
assertContains('font-size: 9.5px', $settingsStyles, 'values usage hints use smaller compact type');
assertContains('font-weight: 560', $settingsStyles, 'values labels are not overly bold');
assertContains('padding: 7px 0;', $settingsStyles, 'values rows stay compact without noisy dividers');
assertContains('font-size: 10.5px; font-weight: 430', $settingsStyles, 'values descriptions are small and light');
assertNotContains('border-bottom: 1px solid color-mix(in srgb, var(--evo-ui-border) 68%, transparent)', $settingsStyles, 'values rows do not add divider noise between fields');
assertContains('.ssettings-compact-row__meta { display: grid; justify-items: end;', $settingsStyles, 'values label column aligns toward controls');
assertContains('.ssettings-compact-row__label { color: var(--evo-ui-text); font-size: 13px; font-weight: 560; line-height: 1.25; text-align: right; }', $settingsStyles, 'values labels right-align on desktop');
assertContains('.ssettings-compact-row__meta { justify-items: start; text-align: left; }', $settingsStyles, 'values labels reset on stacked mobile layout');
...
assertContains('x-bind:disabled="saving || !localDirty || savedClean"', $settingsPanel, 'values save button is disabled while saving, clean, or freshly saved');
...
assertContains('showSavedToast: false', $settingsPanel, 'values save uses shared auto-dismiss toast state');
assertContains('evo-ui-save-toast evo-ui-save-toast--success', $settingsPanel, 'values save renders shared evo-ui toast');
assertContains('x-show="showSavedToast"', $settingsPanel, 'values save toast is shown from persisted save event');
assertContains('2400', $settingsPanel, 'values save toast auto-dismisses after a short delay');
...
assertContains('class="evo-ui-btn__label">@lang(\'evo::global.action_save\')</span>', $settingsPanel, 'values save keeps visible label');
assertContains('x-bind:class="{ \'is-disabled\': saving || !localDirty || savedClean }"', $settingsPanel, 'values save button uses evo-ui disabled class binding');
assertContains('x-bind:data-evo-form-saved="savedClean ? \'true\' : \'false\'"', $settingsPanel, 'values saved-clean marker follows Alpine state');
...
assertContains('ssettings-listbox-input', $settingsPanel, 'values listbox fields use taller listbox styling');
assertContains('size="{{ min(max(count($options), 5), 10) }}"', $settingsPanel, 'values listboxes show at least five option rows');
assertContains('select.evo-ui-input.ssettings-listbox-input:not([multiple]) { min-height: 150px; height: 150px; }', $settingsStyles, 'values single listboxes override evo-ui compact select height');
...
assertContains("route('sSettings.asset', ['file' => 'ssettings.css'])", $indexView, 'manager shell loads sSettings module stylesheet');
...
$configureStyles = (string) file_get_contents(dirname(__DIR__) . '/assets/ssettings.css');
assertContains('data-ssettings-compact-configure', $configurePanel, 'compact configure root');
assertNotContains('<style', $configurePanel, 'configure screen does not contain inline style blocks');
assertContains('data-ssettings-configure-toolbar', $configurePanel, 'compact configure toolbar');
assertContains('class="evo-ui-form-toolbar ssettings-toolbar ssettings-configure-toolbar__right"', $configurePanel, 'configure save uses the same form toolbar context as values save');
...
assertContains('x-bind:disabled="saving || !localDirty || savedClean || @js(!$writable)"', $configurePanel, 'configure save button is disabled while saving, clean, freshly saved, or read-only');
...
assertContains('x-bind:class="{ \'is-disabled\': saving || !localDirty || savedClean || @js(!$writable) }"', $configurePanel, 'configure save button uses evo-ui disabled class binding');
...
assertContains('x-on:evo-ui:form.dirty="markStructureDirty()"', $configurePanel, 'configure save button listens to shared evo-ui DnD dirty events');
...
assertContains('showSavedToast: false', $configurePanel, 'configure save uses shared auto-dismiss toast state');
assertContains('evo-ui-save-toast evo-ui-save-toast--success', $configurePanel, 'configure save renders shared evo-ui toast');
assertContains('x-show="showSavedToast"', $configurePanel, 'configure save toast is shown from persisted save event');
assertContains('2400', $configurePanel, 'configure save toast auto-dismisses after a short delay');
...
assertContains('class="evo-ui-btn__label">@lang(\'evo::global.action_save\')</span>', $configurePanel, 'configure save keeps visible label');
assertContains('data-evo-form-saved="{{ $saved ? \'true\' : \'false\' }}"', $configurePanel, 'configure form exposes saved-clean marker');
assertContains('x-bind:data-evo-form-saved="savedClean ? \'true\' : \'false\'"', $configurePanel, 'configure saved-clean marker follows Alpine state');
...
assertContains('$wire.updateFieldByUid', $configurePanel, 'field modal saves by stable field uid');
...
assertContains('ssettings-options-editor', $configurePanel, 'field modal renders compact option editor');
...
assertContains('optionDragIndex: null', $configurePanel, 'field modal tracks local option drag state');
assertContains('optionHandleIndex: null', $configurePanel, 'field modal only enables option drag after the handle is pressed');
assertContains('optionPlaceholderIndex: null', $configurePanel, 'field modal keeps option placeholder visual state separate from drag state');
assertContains('optionDropIndex: null', $configurePanel, 'field modal tracks the active option insertion position');
assertContains('optionPlaceholderHeight: 38', $configurePanel, 'field modal keeps a stable option placeholder height fallback');
assertContains('moveOption(index, direction)', $configurePanel, 'field modal exposes manual option reorder controls');
assertContains('hoverOption(index, event)', $configurePanel, 'field modal updates option insertion position over rows');
assertContains('hoverOptionList(event)', $configurePanel, 'field modal accepts drops in the option list gaps');
assertContains('dropOptionList(event)', $configurePanel, 'field modal can drop into the option list fallback zone');
assertContains('moveDraggedOption(to)', $configurePanel, 'field modal reorders options through one drop helper');
assertContains('dragOptionStart(index, event)', $configurePanel, 'field modal can start native option drag');
assertContains('if (this.optionHandleIndex !== index)', $configurePanel, 'option row dragstart is guarded so inputs do not start drags');
assertContains('event.dataTransfer.setDragImage', $configurePanel, 'option DnD uses the whole row as its drag preview');
assertContains('this.optionPlaceholderHeight = Math.ceil(box.height)', $configurePanel, 'option DnD placeholder matches the dragged row height');
assertContains('window.requestAnimationFrame(() =>', $configurePanel, 'option placeholder appears after native drag preview is captured');
assertContains('ssettings-option-dnd-placeholder', $configurePanel, 'option DnD uses a physical placeholder like the main list');
assertContains('.ssettings-option-editor-item { display: contents; }', $configureStyles, 'option wrapper does not leave an empty grid gap when the source row is hidden');
assertContains('--ssettings-option-placeholder-height: ${optionPlaceholderHeight}px', $configurePanel, 'option list exposes dragged row height to placeholder CSS');
assertContains('height: var(--ssettings-option-placeholder-height, 38px)', $configureStyles, 'option placeholder uses the measured dragged row height');
assertContains('.ssettings-option-editor-row.is-drag-hidden { position: absolute !important;', $configureStyles, 'dragged option row leaves layout while placeholder reserves its height');
assertContains('dropOption(index, event)', $configurePanel, 'field modal can reorder options on drop');
assertContains('data-ssettings-option-drag-handle', $configurePanel, 'option rows expose a dedicated drag handle');
assertContains('data-ssettings-option-list', $configurePanel, 'option editor exposes a list-level drop fallback');
assertContains('draggable="true"', $configurePanel, 'option rows use the same native draggable row model as the main list');
assertContains('\'is-dragging\': optionPlaceholderIndex === optionIndex', $configurePanel, 'option rows keep source placeholder styling while dragging');
assertContains('\'is-drag-hidden\': optionPlaceholderIndex === optionIndex', $configurePanel, 'option source row is hidden only after drag preview capture');
assertContains('optionDropIndex === optionIndex + 1', $configurePanel, 'option physical placeholder moves after the target row');
assertContains('x-on:pointerdown.capture="cancelOptionHandle($event)"', $configurePanel, 'option rows clear stale handle state when users interact with inputs');
assertContains('x-on:dragstart.stop="dragOptionStart(optionIndex, $event)"', $configurePanel, 'option dragstart is handled on the full row');
assertContains('x-on:dragend.stop="dragOptionEnd()"', $configurePanel, 'option drag state is reset from the full row');
assertContains('x-on:pointerdown.stop="prepareOptionDrag(optionIndex)"', $configurePanel, 'option drag handle arms row dragging on pointerdown');
assertContains('x-on:selectstart.prevent', $configurePanel, 'option drag handle prevents selecting adjacent inputs');
assertContains('pointer-events: none;', $configureStyles, 'option drag handle svg does not steal drag target');
assertContains('draggable="false"', $configurePanel, 'option value inputs do not become drag sources');
assertContains('data-ssettings-option-row', $configurePanel, 'option rows expose dedicated drop targets');
assertContains('window.__sSettingsOptionDrag', $configurePanel, 'option drag state is available to document-level drop guard');
assertContains('ssettings-option-reorder-rail', $configurePanel, 'option rows use the same up/handle/down reorder rail pattern');
...
assertContains('x-on:drop.prevent.stop="dropOption(optionIndex, $event)"', $configurePanel, 'option rows stop drop events before parent DnD');
assertContains('width: min(760px, calc(100vw - 32px))', $configureStyles, 'field modal is wide enough for option controls');
assertContains('grid-template-rows: auto minmax(0, 1fr) auto', $configureStyles, 'field modal reserves fixed header/footer with a scrollable body');
assertContains('max-height: calc(100vh - 32px)', $configureStyles, 'field modal stays inside the viewport when many options are shown');
assertContains('overflow-y: auto', $configureStyles, 'field modal body scrolls long option lists instead of overflowing');
assertContains('overscroll-behavior: contain', $configureStyles, 'field modal body keeps option-list scrolling local');
assertContains('grid-template-columns: 150px minmax(0, 1fr)', $configureStyles, 'option editor aligns label and rows without overflowing');
assertContains('x-on:click="addOption(optionIndex)"', $configurePanel, 'option rows expose add-after action');
assertContains('ssettings-option-actions', $configurePanel, 'option rows use grouped evo-ui row actions');
assertContains('ssettings-option-drag-handle', $configurePanel, 'option drag handle has compact styling');
...
assertContains('type, options', $configurePanel, 'field modal sends options to Livewire save method');
...
assertContains('data-evo-dnd-handle', file_get_contents('/Users/dmi3yy/PhpstormProjects/Extras/evo-ui/views/components/reorder-rail.blade.php'), 'shared reorder rail exposes DnD handle marker');
...
assertContains('.ssettings-compact-field.evo-ui-dnd-row { display: grid;', $configureStyles, 'configure keeps consumer row layout while using shared DnD classes');
assertContains('.ssettings-compact-tab.evo-ui-dnd-group-row { display: grid;', $configureStyles, 'configure keeps consumer tab layout while using shared DnD group classes');
assertNotContains('.ssettings-compact-field.is-drag-hidden { display: none; }', $configureStyles, 'native dnd does not cancel browser drag by display-hiding the source row');
assertContains('.ssettings-compact-fields.is-drag-over .ssettings-compact-empty { display: none; }', $configureStyles, 'empty field copy hides while shared placeholder is active');
assertNotContains('var(--evo-ui-success) 14%', $configureStyles, 'native dnd removes loud green placeholder fill');
assertNotContains('height: 5px; opacity: 1; background: var(--evo-ui-success)', $configureStyles, 'native dnd removes loud insertion stripe');
assertContains(':has(.ssettings-compact-field) .ssettings-compact-empty', $configureStyles, 'empty tab placeholder hides while dragging fields in');
...
assertContains('grid-template-areas: "reorder label actions"', $configureStyles, 'desktop tab grid areas');
assertContains('grid-template-areas: "reorder summary type"', $configureStyles, 'responsive field grid areas');
assertContains('.ssettings-mini-field span,' . PHP_EOL . '.ssettings-modal-field span { justify-self: end;', $configureStyles, 'configure inline labels align toward controls');
assertContains('.ssettings-modal-field span { justify-self: start; text-align: left; }', $configureStyles, 'configure modal labels reset when stacked');
assertContains('.ssettings-mini-field span { justify-self: start; text-align: left; }', $configureStyles, 'configure tab labels reset when stacked');
assertContains('ssettings-type-badge--compact', $configurePanel, 'compact field type badge');
assertContains('title="{{ $types[$fieldType] ?? ucfirst($fieldType) }}"', $configurePanel, 'field type badge exposes full type label');
assertContains('max-width: min(180px, 26vw)', $configureStyles, 'field type badge can expand for long type names');
assertContains('ssettings-system-key', $configurePanel, 'system key chip');
assertContains('evo-ui-row-action--primary', $configurePanel, 'edit action uses evo-ui primary tone');
assertContains('evo-ui-row-action--info', $configurePanel, 'duplicate/add-tab action uses evo-ui info tone');
...
assertContains("@lang('evo::global.action_save')", $configurePanel, 'modal primary action says save');
